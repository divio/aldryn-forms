...
class EmailStorageBackend(BaseStorageBackend):
    verbose_name = _('Email Only Storage')

    def form_valid(self, cmsplugin, instance, request, form):
        cmsplugin.send_notifications(instance, form)
        form_id = form.form_plugin.id
        logger.info('Not persisting data for "{}" since storage_backend is set to "email_storage'.format(form_id))
...
class NoStorageBackend(BaseStorageBackend):
    ...
    def form_valid(self, cmsplugin, instance, request, form):
        form_id = form.form_plugin.id
        logger.info('Not persisting data for "{}" since storage_backend is set to "no_storage'.format(form_id))
